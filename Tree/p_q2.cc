...
struct S {
    u32 m_max = 0;
    void pull(const S &l, const S &r) { m_max = std::max(l.m_max, r.m_max); }
};
...
int main()
{
    ...
    for (auto &x : initial_values) cin >> x.m_max;
    ...
    while ( q--)
    {
        ...
        if (qt == '1') {
            ...
            u32 ans = 0;
            hld(u, v,
                [&](u32 l, u32 r) { ans = std::max(ans, st.fold(l, r).m_max); });
            std::cout << ans << ' ';
        }
    }
}