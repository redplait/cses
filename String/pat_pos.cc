...
// solution for Pattern Positions
// suffex tree implementation borrowed from https://github.com/Rerito/suffix-tree/blob/master/suffixtree.h
// 0.25s
...
class SuffixTree {
    ...
    typedef std::tuple<Node*,index_type, index_type, int> ReferencePoint;
    ...
    ReferencePoint update(Node *n, MappedSubstring ki) {
        ...
        ReferencePoint sk(n, ki.ref_str, ki.l, 0);
        ki1.r = ki.r-1;
    ...
    ReferencePoint canonize(Node *n, MappedSubstring kp) {
        if (kp.r < kp.l)
            return ReferencePoint(n, kp.ref_str, kp.l, 0);
    ...
    index_type get_starting_node(const string& s, ReferencePoint *r) {
        ...
        while (!s_runout) {
            ...
            if (nullptr != t.tgt) {
#ifdef DEBUG
printf("l %ld r %ld k %ld\n", t.sub.l, t.sub.r, k);
#endif
                std::get<3>(*r) = t.sub.l - k;
                index_type i;
...
public:
    ...
    bool is_suffix(InputIterator const & str_begin, InputIterator const & str_end) {
        auto s = make_string(str_begin, str_end);
        ReferencePoint root_point(&tree.root, -1, 0, 0);
    ...
    bool is_substring(InputIterator const & str_begin, InputIterator const & str_end) {
        auto s = make_string<InputIterator, false>(str_begin, str_end);
        ReferencePoint root_point(&tree.root, -1, 0, 0);
        return (get_starting_node(s, &root_point) == std::numeric_limits<index_type>::max());
    }

    template <typename InputIterator>
    int get_pos(InputIterator const & str_begin, InputIterator const & str_end) {
        auto s = make_string<InputIterator, false>(str_begin, str_end);
        ReferencePoint root_point(&tree.root, -1, 0, 0);
        auto pos = get_starting_node(s, &root_point);
        if ( pos != std::numeric_limits<index_type>::max() ) return -1;
// printf("<%ld %ld %ld>\n", pos, std::get<1>(root_point), std::get<2>(root_point));
        return 1+std::get<3>(root_point);
    }
